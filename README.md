...
GitHub 저장소에 올린 mp3 파일을 브라우저에서 간단히 재생할 수 있는 개인용 정적 음악 플레이어입니다. 백엔드 없이 HTML, CSS, JavaScript만 사용하므로 GitHub Pages 또는 로컬 브라우저에서 바로 열 수 있습니다.
...
`sample1.mp3`, `sample2.mp3` 파일은 예시 이름입니다. 이 프로젝트에는 실제 mp3 파일을 포함하지 않았으므로, `music` 폴더에 직접 mp3 파일을 넣어 주세요.

## 노래 추가 방법

1. `music` 폴더에 mp3 파일을 넣습니다.
...
파일 경로는 반드시 `./music/파일명.mp3` 형태로 작성합니다. 파일명은 대소문자까지 실제 mp3 파일명과 같아야 합니다.
...
- 큰 mp3 파일은 GitHub 저장소에 올리기 적합하지 않습니다.
- 본인이 직접 만든 파일 또는 사용 권한이 있는 음악 파일만 사용해야 합니다.
- 공개 저장소에 올리면 mp3 파일도 공개될 수 있습니다.
